	...
	public MultiTickTask generate(Area area) {
		...
		for (int x = 0; x < area.getWidth(); x+=32) {
			for (int z = 0; z < area.getLength(); z+=32) {
				int xPos = x;
				int zPos = z;
				task.add(() ->BasicGenUtil.generateCube(area.getPos().clone().add(xPos, 0, zPos), Material.AIR, (int) Math.min(32, area.getWidth() - xPos), (int)area.getHeight(), (int) Math.min(32, area.getLength() - zPos)));
			}
		}
		//Stone Cube
		for (int x = 0; x < area.getWidth(); x+=32) {
